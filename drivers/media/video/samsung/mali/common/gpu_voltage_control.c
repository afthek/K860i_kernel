...
static int gpu_under_voltage = 1;
...
static ssize_t gpu_voltage_show(struct device *dev, struct device_attribute *attr, char *buf) {
	int i;
	if(gpu_under_voltage)
	{
		gpu_under_voltage = 0;
		for( i = 0; i < 4; i++ ) {
			mali_dvfs[i].vol -= 50000;
			if (mali_dvfs[i].vol < MIN_VOLTAGE_GPU) {
				mali_dvfs[i].vol = MIN_VOLTAGE_GPU;
			}
			else if (mali_dvfs[i].vol > MAX_VOLTAGE_GPU) {
				mali_dvfs[i].vol = MAX_VOLTAGE_GPU;
			}
		}
	}
