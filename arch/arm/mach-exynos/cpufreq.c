		...
		unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
        ...
        //jeff, make sure sleep VDD_ARM > 1.1V
#if (defined(CONFIG_MACH_STUTTGART) && defined(SAFE_ARM_SLEEP_VOLT_PATCH))
        {
            unsigned int cur_vol = 0;
            cur_vol = regulator_get_voltage(arm_regulator);
            if(cur_vol > 0 && cur_vol < SAFE_ARM_SLEEP_VOLTAGE)
                regulator_set_voltage(arm_regulator, SAFE_ARM_SLEEP_VOLTAGE, SAFE_ARM_SLEEP_VOLTAGE + 25000);
            else
                pr_info("cur_vdd_arm=%duv, Don't set vdd_arm = %duv\n", cur_vol, SAFE_ARM_SLEEP_VOLTAGE);
        }
#endif
